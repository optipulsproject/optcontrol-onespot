...
These phenomena are often modeled in terms of the classical Stefan problem \cite{Gupta:2003:1}, which is a particular kind of a boundary value problem describing the evolution of a moving boundary between two phases of a material undergoing a phase change. 
...
Another effect to be taken into account is that the \emph{density} of aluminum alloys changes significantly over the temperature regime under consideration due to thermal expansion and contraction.
...
\end{equation}
respectively \added[id=DS]{\textbf{[reference to these models needed]}}.
...
To find the optimal control we apply the \emph{projected gradient descent} procedure \added[id=DS]{\textbf{[citation needed]}}. Since the procedure is well known, we present only a short general outline, one can find more details in \cite[\texttt{optipuls.optimization}]{optipuls_github}.
...
\begin{figure}
	...
	\DontPrintSemicolon
	\KwIn{$u_\text{initial}$}
	\KwOut{$u_\text{optimized}$}
	$u_\text{current} \gets u_\text{initial}$\;
	\While{$ \| J_u (\mathcal{P}_{U} u_\text{current}) \| < \text{tolerance} $}{
		compute the gradient of the objective $J_u(u_\text{current})$\;
		\Repeat{$J(u_\text{trial}) \ge J(u_\text{current}) - \sigma \alpha \|J_u (u_\text{current})\|^2$}{
			line search for the step size $\alpha$\;
			$u_\text{trial} \gets \mathcal{P}_{[0,1]} (u_\text{current} - \alpha J_u (u_\text{current}))$\;
		}
		$u_\text{current} \gets u_\text{trial}$\;
	}
	\end{algorithm}
...
% \begin{itemize}
% 	\item short description of the method itself
% 	\item spell out the Armijo condition
% \end{itemize}
% We accept $u_\text{trial}$ if the following Armijo condition is satisfied
% \begin{equation} \label{eq:Armijo_cond}
%  	J(u_\text{trial}) < J(u_\text{current}) - \sigma \cdot \text{step}_\text{current} \cdot \|(\grad_u J)(u_\text{current})\|^2
% \end{equation}
...
The conventional pulse laser welding uses a rectangular laser pulse shape, i.e. the laser is working full power for a short time and is switched of immediately after. However in welding of aluminum alloys this always leads to hot-cracking. A so-called linear-rampdown pulse shape, i.e. when the laser power is decreasing linearly after a short period of working full power, Has shown its potential to maintain a crack-free welding of aluminum alloys \added[id=DS]{\textbf{[citation needed]}}.
