...
\begin{equation} \label{eq:J_penetration_discrete}
	...
	\paren[auto]\{\}{\paren[Bigg](){\added[id=RH]{\tau} \sum_{n=1}^{N_t} \abs[big]{\theta_n(0,z_\textup{target})}^p}^{1/p} - \theta_\textup{target}}^2
	.
\end{equation}
...
\begin{equation}
	...
	\replaced[id=DS]{\chi_n(r, z)}{\chi(\theta_n)}
	\, r \d r \d z
\end{equation}
\added[id=DS]{
...
\begin{equation}
	...
	\coloneqq
	\begin{cases}
		1 & \text{if}\ \text{solidus} \le \theta_n\ \text{and}\ \theta_{n+1} < \text{liquidus}
		,
		\\
		0 & \text{otherwise}
		.
...
% ge(theta_k, solidus), lt(theta_kp1, liquidus)
