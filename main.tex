...
To derive the discrete version of $J_\text{penetration}$, we discretize \eqref{eq:J_penetration} with $l^p$-norm taking sufficiently large $p$
\begin{equation}
	J_\text{penetration} 
	= 
	\frac{\beta_\text{penetration}}{2}
	\paren[auto]\{\}{\paren[Bigg](){\added[id=RH]{\tau} \sum_{n=1}^{N_t} \abs[big]{\theta_n(0,z_\text{target}\added[id=RH]{,n\tau})}^p}^{1/p} - \theta_\text{target}}^2
	.
\end{equation}
...
\begin{equation}
	J_\text{velocity} 
	=
	\frac{\beta_\text{velocity}}{2} \tau \sum_{n=0}^{N_t-1} \int_{\omega} \max \paren[big]\{\}{ \replaced[id=RH]{v(\theta_n, \theta_{n+1}) - v_{\max}}{- v_{\max} - v(\theta_n, \theta_{n+1})}, \; 0}^2 \cdot \chi(\theta_{n+\alpha}) \, r \d r \d z
\end{equation}
\added[id=RH]{\textbf{@DS: The above corrections in the sign of $v(\theta_n,\theta_{n+1})$ may require an update of the code.}}

The remaining penalty terms $J_\text{completeness}$ and $J_\text{control}$ are discretized according to
\begin{align}
	J_\text{completeness} 
	&
	=
	\frac{\beta_\text{completeness}}{2} \int_{\omega} \max \paren[big]\{\}{\theta_{N_t} - \text{solidus}, \; 0}^2 \, r \d r \d z
	, 
	\\
	J_\text{control} 
	&
	=
	\frac{\beta_\text{control}}{2} \tau \sum_{n=0}^{N_t-1} u_n^2
	.
\end{align}
