...
The numerical experiments provided in this section share the following parameters (see \cite[\texttt{env/problem.py}]{optcontrol_github}):

\added[id=DS]{\textbf{I will work on the layout of the listed parameters.}}

\begin{description}
	\item [space domain] \mbox{} \\
		\begin{tabular}{rl}
			radius of the cylinder $\Omega$ & $\SI{2.5}{\mm}$ \\
			height of the cylinder $\Omega$ & $\SI{0.5}{\mm}$ \\
			radius of the laser beam $\Omega$ & $\SI{0.2}{\mm}$
		\end{tabular}

	\item [equation and boundary conditions] \mbox{} \\
		\begin{tabular}{rl}
			ambience temperature & $\theta_\text{amb} = \SI{295}{\K}$ \\
			convective heat transfer coefficient & $h = 20$ \\
			radiative heat transfer coefficient & $k = 2.26 \cdot 10^{-9}$
		\end{tabular}

	\item [objective functional] \mbox{} \\
		\begin{tabular}{rl}
	...
	\item [material properties] \mbox{} \\
		\begin{tabular}{rl}
			solidus point & $\SI{858}{\K}$ \\
			liquidus point & $\SI{923}{\K}$ \\
			enthalpy of fusion & $\SI{397000}{\J}$ \\
			coefficients $s(\theta)$ and $\kappa(\theta)$ & discussed in section~\ref{sec:modelling}, see Figure~\ref{fig:coef}
		\end{tabular}
\end{description}
